export const translations = {
    en: {
        readyToBloom: "Ready to",
        bloom: "Bloom?",
        tagline: "Let's chat, listen, and grow your vocabulary together.",
        launchBtn: "Launch Journey",
        installBtn: "Install to Home Screen",
        powerMeUp: "Power Me Up! ✨",
        apiHelp: "To start our journey, I need a tiny bit of help. Please grab your xAI Grok API Key from their dashboard.",
        step1: "Visit xAI Console",
        step2: "Create a new API Key",
        step3: "Paste it below and let's bloom!",
        unlockPotential: "Unlock My Potential",
        validating: "Validating...",
        exploreLater: "Maybe later, just let me explore ✨",
        stayClose: "Stay Close! 💖",
        pwaInvite: "I'm verified and ready. Install me on your home screen for the fastest access to your learning journey!",
        installSmartLern: "Install Smart-Lern",
        letsChat: "Let's Chat!",
        maybeLater: "Maybe later",
        settingsPanel: "Settings Panel",
        groqIntelligence: "Grok (xAI) Intelligence",
        verifyKey: "Verify Key",
        verifying: "Checking...",
        verified: "Verified ✓",
        voiceEngine: "Voice Engine",
        speechVelocity: "Speech Velocity",
        slow: "SLOW",
        fast: "FAST",
        applyChanges: "Apply Changes",
        exit: "EXIT",
        exploreMode: "Explore Mode",
        localEngine: "Local Engine",
        connected: "Connected",
        typeReply: "Type your reply...",
        devModeAlert: "Dev Mode: Static exploration active. Add API key in settings when ready!",
        language: "Language",
        arabic: "العربية",
        english: "English",
        cloudVoice: "ElevenLabs Voice (Pro)",
        elevenApiKey: "ElevenLabs API Key",
        elevenHelp: "Recommended: Use ElevenLabs for the most realistic voices. No credit card needed!",
        saveKey: "Save Key",
        notSet: "Not set (using browser)",
        clickHere: "Click here",
        setupGuide: "Setup Guide",
        howToGetKeys: "How to get API Keys?",
        groqTitle: "1. Get Grok API Key (Intelligence)",
        groqStep1: "Visit the xAI Console (console.x.ai).",
        groqStep2: "Create a new API Key (give it a name like 'SmartLern').",
        groqStep3: "Copy the key (starts with 'xai-') and paste it in Settings.",
        elevenTitle: "2. Get ElevenLabs Key (Premium Voice)",
        elevenStep1: "Go to ElevenLabs.io and sign up (No credit card needed).",
        elevenStep2: "Click on your 'Profile Icon' at the bottom left.",
        elevenStep3: "Copy your 'API Key' and paste it in Settings.",
        back: "Back",
        groqGuide: {
            title: "Setup Intelligence (Grok xAI)",
            subtitle: "Give your AI Elon Musk's Grok brain",
            whatIs: "An API Key is a 'Digital Key' that lets our app talk to Elon Musk's Grok. Without it, the AI cannot answer your questions.",
            step1: "1. Go to the xAI Console (Click the button below).",
            step2: "2. Sign in with your X account or email.",
            step3: "3. Create a new API Key.",
            step4: "4. Give it a name like 'My Smart Lern'.",
            step5: "5. Copy the long code. It usually starts with 'xai-'. Keep it secret!"
        },
        elevenGuide: {
            title: "Setup Voice (ElevenLabs)",
            subtitle: "The most realistic AI voices ever",
            whatIs: "ElevenLabs is the world leader in AI voices. Their voices sound exactly like real people, and you can get started for FREE without any credit card!",
            step1: "1. Visit ElevenLabs.io (Click the button below).",
            step2: "2. Sign up using your Google account or just an email.",
            step3: "3. Click your 'Profile' icon at the bottom left of the screen.",
            step4: "4. You will see your API Key there. Click the 'Eye' icon to see it, then copy it.",
            step5: "5. Paste the key in the Smart-Lern settings and enjoy the magic!"
        }
    },
    ar: {
        readyToBloom: "هل أنت مستعد لـ",
        bloom: "تزهر؟",
        tagline: "دعنا نتحدث، نستمع، وننمي مفرداتك معاً.",
        launchBtn: "ابدأ الرحلة",
        installBtn: "تثبيت على الشاشة الرئيسية",
        powerMeUp: "فعّل ذكائي! ✨",
        apiHelp: "لبدء رحلتنا، أحتاج إلى مساعدة بسيطة. يرجى الحصول على مفتاح Grok (xAI) من لوحة التحكم الخاصة بهم.",
        step1: "قم بزيارة لوحة تحكم xAI",
        step2: "أنشئ مفتاح API جديد",
        step3: "ألحقه أدناه ولنبدأ الإزهار!",
        unlockPotential: "أطلق قدراتي",
        validating: "جاري التحقق...",
        exploreLater: "ربما لاحقاً، دعني أستكشف فقط ✨",
        stayClose: "كن قريباً! 💖",
        pwaInvite: "لقد تم التحقق وأنا جاهزة. ثبتني على شاشتك الرئيسية للوصول الأسرع لرحلة تعلمك!",
        installSmartLern: "تثبيت Smart-Lern",
        letsChat: "لنبدأ الدردشة!",
        maybeLater: "ربما لاحقاً",
        settingsPanel: "لوحة الإعدادات",
        groqIntelligence: "ذكاء Grok (xAI)",
        verifyKey: "تحقق من المفتاح",
        verifying: "جاري الفحص...",
        verified: "تم التحقق ✓",
        voiceEngine: "محرك الصوت",
        speechVelocity: "سرعة الكلام",
        slow: "بطيء",
        fast: "سريع",
        applyChanges: "حفظ التغييرات",
        exit: "خروج",
        exploreMode: "وضع الاستكشاف",
        localEngine: "محرك محلي",
        connected: "متصل",
        typeReply: "اكتب ردك هنا...",
        devModeAlert: "وضع التطوير: الاستكشاف الثابت نشط. أضف مفتاح API في الإعدادات عندما تكون مستعداً!",
        language: "اللغة",
        arabic: "العربية",
        english: "English",
        cloudVoice: "صوت ElevenLabs (احترافي)",
        elevenApiKey: "مفتاح ElevenLabs API",
        elevenHelp: "موصى به: استخدم ElevenLabs لأكثر الأصوات واقعية في العالم. لا حاجة لبطاقة بنكية!",
        saveKey: "حفظ المفتاح",
        notSet: "غير مفعل (يتم استخدام المتصفح)",
        clickHere: "اضغط هنا",
        setupGuide: "دليل الإعداد",
        howToGetKeys: "كيف أحصل على المفاتيح؟",
        groqTitle: "1. الحصول على مفتاح Grok (للذكاء)",
        groqStep1: "قم بزيارة لوحة تحكم xAI Console.",
        groqStep2: "أنشئ مفتاح API جديد (سمِّه SmartLern).",
        groqStep3: "انسخ المفتاح (يبدأ بـ 'xai-') وضعه في الإعدادات.",
        elevenTitle: "2. مفتاح ElevenLabs (للصوت الفخم)",
        elevenStep1: "اذهب إلى ElevenLabs.io وسجل حساباً (مجاناً وبدون بطاقة بنكية).",
        elevenStep2: "اضغط على أيقونة ملفك الشخصي في أسفل يسار الشاشة.",
        elevenStep3: "انسخ مفتاح الـ API وضعه في الإعدادات.",
        back: "رجوع",
        groqGuide: {
            title: "إعداد الذكاء (Grok xAI)",
            subtitle: "امنح تطبيقك عقل Grok الخاص بإيلون ماسك",
            whatIs: "مفتاح الـ API هو 'مفتاح رقمي' يسمح لتطبيقنا بالتحدث مع ذكاء Grok الاصطناعي. بدون هذا المفتاح، لن يتمكن الذكاء الاصطناعي من الإجابة على أسئلتك.",
            step1: "1. اذهب إلى لوحة تحكم xAI Console (اضغط على الزر أدناه).",
            step2: "2. قم بتسجيل الدخول باستخدام حساب X (تويتر سابقاً).",
            step3: "3. أنشئ مفتاح API جديد.",
            step4: "4. اضغط على زر 'Create API Key'. أعطه اسماً مثل 'My Smart Lern'.",
            step5: "5. انسخ الكود الطويل الذي سيظهر. عادة ما يبدأ بـ 'xai-'. حافظ عليه سراً!"
        },
        elevenGuide: {
            title: "إعداد الصوت (ElevenLabs)",
            subtitle: "أكثر الأصوات واقعية في العالم",
            whatIs: "ElevenLabs هي الشركة الرائدة عالمياً في أصوات الذكاء الاصطناعي. أصواتهم تبدو تماماً مثل البشر الحقيقيين، ويمكنك البدء مجاناً وبدون أي بطاقة بنكية!",
            step1: "1. قم بزيارة موقع ElevenLabs.io (بالضغط على الزر أدناه).",
            step2: "2. سجل حساباً باستخدام جوجل أو إيميلك الشخصي.",
            step3: "3. اضغط على أيقونة 'الملف الشخصي' (Profile) في أسفل يسار الشاشة.",
            step4: "4. ستجد مفتاح الـ API هناك. اضغط على علامة 'العين' لرؤيته، ثم انسخه.",
            step5: "5. ضع المفتاح في إعدادات Smart-Lern واستمتع بالسحر!"
        }
    }
};
